...
BEGIN
	...
	VALUES (VAR_CEDULA, VAR_NOMBRE, VAR_ROL, VAR_CLAVE, VAR_PREGUNTA, VAR_PREGUNTA, VAR_USUARIO_ACTIVADO, VAR_USUARIO_VERIFICADO, NOW(), NOW(), VAR_CONTACTO);
	SELECT "NUEVO USUARIO REGISTRADO" AS "RESULTADO";
END $$
DELIMITER ;
...
BEGIN
	...
	IF(DEC_NUMEROCOMPRAS = 0) THEN
		SET DEC_NUMEROCOMPRAS = (SELECT COUNT(CEDULA_USUARIO) FROM HISTORIAL WHERE CEDULA_USUARIO = VAR_CEDULA);
	END IF;
	...
	ELSE 
		SELECT "EL USUARIO NO SE PUEDE BORRAR YA QUE PODRIA AFECTAR GRAVEMENTE LOS DATOS ALMACENADO"  AS "RESULTADO";
	END IF;
END $$
DELIMITER ;
...
-- MOFDIFICAR USUARIO --
DROP PROCEDURE IF EXISTS `PROCEDURE_MODIFICAR_USUARIO`;

DELIMITER $$
CREATE PROCEDURE `PROCEDURE_MODIFICAR_USUARIO`
	(IN VAR_CEDULA DOUBLE,IN VAR_NOMBRE VARCHAR(700),IN VAR_CLAVE VARCHAR(2000),IN VAR_PREGUNTA VARCHAR(500),IN VAR_RESPUESTA VARCHAR(2000),IN VAR_CONTACTO VARCHAR(1000))
BEGIN
	DECLARE DEC_VACIO DOUBLE;
	SET DEC_VACIO = (SELECT IFNULL(VAR_CEDULA, 0));
	IF(DEC_VACIO != 0) THEN
		UPDATE USUARIO
		SET NOMBRE = VAR_NOMBRE, CLAVE = VAR_CLAVE, PREGUNTA = VAR_PREGUNTA, RESPUESTA = VAR_PREGUNTA, CONTACTO = VAR_CONTACTO
		WHERE CEDULA = VAR_CEDULA;
		SELECT CONCAT("USUARIO ", VAR_CEDULA, " MODIFICADO") AS "RESULTADO";
	END IF;
END $$
